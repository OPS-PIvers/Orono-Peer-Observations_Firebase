...
/** Outcome of a cancellation, surfaced to callers for audit/return. */
...
/**
 * Core cancellation: free a booked slot back to the pool and tear down the
 * Draft observation + calendar events + ledger interval. Authorization is the
 * caller's responsibility — this helper is reused both by the `cancelBooking`
 * callable (after it checks the caller) and by system-initiated cancellations
 * (e.g. a bell-schedule change that removed a booked period), which run as
 * `'system'`.
 *
 * Mirrors the callable's transaction exactly, then best-effort recomputes
 * blocked slots, deletes the still-Draft observation, sends the cancellation
 * email, and writes an audit row. `requireAuth` lets the callable enforce its
 * own permission rules inside the transaction before any write happens.
 */
export async function cancelBookingForSlot(args: {
  db: Firestore;
  scheduling: SchedulingSettings;
  windowId: string;
  slotId: string;
  reason: string;
  /** Audit `userEmail` for who/what initiated the cancellation. */
  initiatedBy: string;
  /** Run inside the transaction after the slot is confirmed booked; throw to
   *  abort (e.g. permission denied). The callable uses this to gate access. */
  authorize?: (ctx: { window: ObservationWindow; slot: ObservationSlot }) => void;
}): Promise<CancelBookingResult> {
  const { db, scheduling, windowId, slotId, reason, initiatedBy, authorize } = args;

  const windowRef = db.collection(COLLECTIONS.observationWindows).doc(windowId);
  const slotRef = windowRef.collection(WINDOW_SUBCOLLECTIONS.slots).doc(slotId);
  ...
  await recomputeBlockedSlots(db, windowId).catch((err: unknown) =>
  ...
  if (scheduling.cancellationEmailEnabled && staffEmail) {
    ...
    await sendTemplatedEmail({
      ...
      mailDocId: `scheduling.bookingCancelled-${windowId}-${slotId}-${Date.now().toString()}`,
      auditDetails: {
        windowId,
        slotId,
        triggerType: 'scheduling.bookingCancelled',
  ...
  await db.collection(COLLECTIONS.auditLog).add({
    ...
    userEmail: initiatedBy,
    action: AUDIT_ACTIONS.slotCancelled,
    target: `${COLLECTIONS.observationWindows}/${windowId}/${WINDOW_SUBCOLLECTIONS.slots}/${slotId}`,
    details: {
      windowId,
      slotId,
      reason,
  ...
  return { cancelled: true, slot, window, staffEmail, observationId };
}
