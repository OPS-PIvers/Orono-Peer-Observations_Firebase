...
/**
 * A 6-digit `#RRGGBB` hex — the only shape {@link derivePrimaryShades} can
 * safely parse. `useBranding()` resolves an unset/undefined color to the OPS
 * default, but a Firestore doc written before save-time validation existed can
 * still hold an arbitrary string, so the provider re-checks the shape and
 * treats anything else as "unbranded" (no override, stock palette applies).
 */
const PRIMARY_COLOR_HEX_PATTERN = /^#[0-9a-fA-F]{6}$/;
 ...
 *
 * When the color is the stock OPS blue — or any non-hex value that slipped
 * past validation — the overrides are removed entirely so the exact DESIGN.md
 * token values apply.
 *
...
export function BrandingProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (
      !PRIMARY_COLOR_HEX_PATTERN.test(normalized) ||
      normalized === OPS_BRAND.defaultPrimaryColor
    ) {
      clear();
    } else {
      const shades = derivePrimaryShades(normalized);
