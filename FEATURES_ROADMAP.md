...
**Why this fits.** `resendStaffInvite` is an existing admin-only callable, and the district is mid-rollout toward an Aug/Sept 2026 cutover where exactly this kind of adoption-tracking view is highest-value.

> **Correction (2026-07-29):** this brief originally claimed `sign_in` was "a real AUDIT_ACTIONS entry written on every successful login." That was false when written — `sign_in` existed only in its own schema definition (`packages/shared/src/schema/auditLog.ts`) and in a rules test; nothing in the app ever wrote it, so an audit-log-backed implementation would have recovered zero history. The implementation (PR #78) uses option (b) below, sourced from Firebase Auth's own user records (`getAuth().listUsers()`, which carries real `metadata.lastSignInTime` history) for the one-time backfill, and also fixed `syncMyClaims` to finally write the `sign_in` audit entry going forward so this stops being vestigial.

**Implementation.** Add a query (client-side, admin-only route so firestore.rules' `allow read: if isAdmin()` on /auditLog already covers it) for the most recent `sign_in` audit entry per staff email — since Firestore can't do a 'group by userEmail, latest timestamp' query directly, either (a) do it in a new admin-only callable that reads /staff (active) and /auditLog with `action == 'sign_in'` via the Admin SDK and diffs the sets, returning `{ email, name, invitedAt }[]` for staff with zero sign-in entries, or (b) simpler/cheaper: denormalize a `lastSignInAt` field onto the /staff doc — add a write in whatever function currently handles first sign-in (check syncMyClaims.ts, apps/functions/src/auth/syncMyClaims.ts) so `staff/{email}.lastSignInAt` gets stamped, then the dashboard card is a plain Firestore query for `isActive == true` filtered client-side for a null-or-missing `lastSignInAt` (an equality filter on `lastSignInAt == null` would silently miss any doc that omits the field). Prefer (b) for a much cheaper live-query dashboard card. Card renders in an existing admin dashboard component (apps/web/src/admin/dashboard/) with a 'Resend invite' button wired to the existing `resendStaffInvite` callable already used elsewhere.
